...
// Remove the active parent and child themes from the check
	...
	class Northeasternwebteam_Page_Navigation_Walker extends Walker_Nav_Menu {
    	function display_element( $element, &$children_elements, $max_depth, $depth=0, $args, &$output ) {
        	$id_field = $this->db_fields['id'];
	        if ( !empty( $children_elements[ $element->$id_field ] ) ) {
    	        $element->classes[] = 'northeasternwebteam-menu-item-parent';
        	}
	...
	*/
	 function northeasternwebteam_nav_menu_args( $args ) {
    	if ( 1 !== $args[ 'depth' ] && has_nav_menu( 'menu_location' ) ) {
        	$args[ 'walker' ] = new Northeasternwebteam_Page_Navigation_Walker;
	    }
    	return $args;
	}
	add_filter( 'wp_nav_menu_args', 'northeasternwebteam_nav_menu_args' );
